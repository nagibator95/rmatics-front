...
import {IContest} from '../../core/stores/contest/types/contest.types';

import {MessageResolverService} from './message-resolver.service';

@Injectable()
export class AnotherContestResolverService implements Resolve<IContest> {
    constructor(
    ...
    waitForContestToDownload(problemId: number): Observable<IContest> {
        return this.store$.pipe(select(ContestSelectors.getContest())).pipe(
            ...
            tap(contest => {
                ...
                if (!this.message.isNavigated) {
                    this.router
                        .navigate([
                            problems.filter(problem => problem.id === problemId)[0].href,
                        ])
