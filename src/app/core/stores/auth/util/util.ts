...
export const formatData = (response: IApiResponse<IApiAuth>): IFormattedApiResponse => {
...
export const setCookies = (item: ICookies) => {
    Object.keys(item)
...
export const setTokenResponseToCookies = (item?: IAuthData, rememberMe?: boolean) => {
    const rememberData =
        item !== undefined && Boolean(rememberMe)
            ? {
                  login: item.login,
